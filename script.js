...
let autoClosingModal = false;

function saveDraftBeforeClosing(name) {
  if (autoClosingModal) return;

  if (name === "noteModal" && $("noteModal").classList.contains("show")) {
    const hasDraft = $("noteTitle").value.trim() || $("noteTags").value.trim() || $("noteContent").value.trim();
    if (hasDraft) {
      autoClosingModal = true;
      saveCurrentNote(false);
      autoClosingModal = false;
    }
  }

  if (name === "playlistModal" && $("playlistModal").classList.contains("show")) {
    const nameValue = $("playlistName").value.trim();
    if (nameValue) {
      autoClosingModal = true;
      savePlaylist();
      autoClosingModal = false;
...
function hideModal(name) {
  // Outside tap, X and Cancel all preserve a real draft before leaving.
  saveDraftBeforeClosing(name);
...
function saveCurrentNote(closeAfterSave = true) {
  const title = $("noteTitle").value.trim();
  const tags = $("noteTags").value.trim();
  const content = $("noteContent").value;

  // Do not create an empty note when the user simply backs out of a blank editor.
  if (!editingId && !title && !tags && !content.trim()) {
    if (closeAfterSave) hideModal("noteModal");
    return false;
  }

  let n = editingId ? notes.find(x => String(x.id) === String(editingId)) : null;
  if (!n) {
    n = { id: id(), favorite:false, pinned:false, hidden:false, created:Date.now() };
    notes.unshift(n);
    editingId = n.id;
  }
...
function openPlaylistModal(pid = null) {
  ...
  document.querySelector("#playlistModal .modal-top h2").textContent = existing ? "Edit Playlist" : "Create Playlist";

  // System playlists are generated automatically. Their names should not be changed.
  const isSystem = existing && (existing.name === "Favorites" || existing.name === "Pinned");
  $("playlistName").disabled = !!isSystem;
  $("playlistName").placeholder = isSystem ? "System playlist name cannot be changed" : "Playlist name...";

  const visibleNotes = notes.filter(n => n.hidden !== true);
  ...
  $("playlistNoteSelector").innerHTML = visibleNotes.length
    ? visibleNotes.map(n => `<label class="selector-item"><input type="checkbox" value="${n.id}" ${selectedIds.has(String(n.id)) ? "checked" : ""}> ${escapeHtml(n.title)}</label>`).join("")
...
function savePlaylist() {
  ...
  // A blank playlist name is the only reason creation is refused.
  if (!name) {
    $("playlistName").focus();
    return false;
  }

  const selected = [...$("playlistNoteSelector").querySelectorAll("input:checked")].map(x => String(x.value));

  if (editingPlaylistId) {
    const playlist = playlists.find(p => String(p.id) === String(editingPlaylistId));
    if (playlist) {
      const isSystem = playlist.name === "Favorites" || playlist.name === "Pinned";
      if (!isSystem) {
        const duplicate = playlists.some(p =>
...
$("savePlaylistBtn").addEventListener("click", event => {
  event.preventDefault();
  event.stopPropagation();
  savePlaylist();
});
...
document.addEventListener("click", event => {
  ...
  if (playlistCard) {
    // The options button is handled separately and must not open the playlist underneath it.
