...
function itemMenuHtml(type, id) {
  return `<div class="item-menu-wrap">
    <button class="item-menu-btn" type="button" data-menu-toggle>⋮</button>
    <div class="item-menu-popup">
      <button type="button" data-item-action="edit" data-item-type="${type}" data-item-id="${id}">✏️ Edit</button>
      <button type="button" class="delete-option" data-item-action="delete" data-item-type="${type}" data-item-id="${id}">🗑️ Delete</button>
    </div>
  </div>`;
}
function closeAllItemMenus(){
  document.querySelectorAll(".item-menu-popup.show").forEach(m=>m.classList.remove("show"));
}
function deleteNoteById(noteId){
  if(!confirm("⚠️ Are you sure you want to permanently delete this note? This cannot be undone.")) return;
  const target=String(noteId);
  notes=notes.filter(n=>String(n.id)!==target);
  playlists.forEach(p=>{if(Array.isArray(p.noteIds))p.noteIds=p.noteIds.map(String).filter(id=>id!==target);});
  saveData(); render(); renderHiddenNotes();
  hideModal("noteModal"); hideModal("longPressMenu");
}
function deletePlaylistById(playlistId){
  const p=playlists.find(x=>String(x.id)===String(playlistId));
  if(!p)return;
  if(!confirm(`⚠️ Are you sure you want to delete "${p.name}"? The notes inside it will NOT be deleted.`))return;
  playlists=playlists.filter(x=>String(x.id)!==String(playlistId));
  saveData(); render(); hideModal("playlistViewModal");
}
function editPlaylistById(playlistId){
  const p=playlists.find(x=>String(x.id)===String(playlistId));
  if(!p)return;
  const name=prompt("Edit playlist name:",p.name);
  if(name===null)return;
  const clean=name.trim();
  if(!clean){alert("Playlist name cannot be empty.");return;}
  p.name=clean; saveData(); render();
}
...
function render() {
  ...
  $("notesList").innerHTML = filtered.length ? filtered.map(n => `
    <article class="note-card ${n.pinned ? "pinned" : ""}" data-note="${n.id}">${itemMenuHtml("note", n.id)}
...
document.addEventListener("click",(event)=>{
  ...
  if(toggle){
    ...
    const popup=toggle.parentElement.querySelector(".item-menu-popup");
    const open=popup.classList.contains("show");
    closeAllItemMenus(); if(!open)popup.classList.add("show");
    return;
  }
  const btn=event.target.closest("[data-item-action]");
  if(btn){
    event.preventDefault();event.stopPropagation();closeAllItemMenus();
    const type=btn.dataset.itemType, action=btn.dataset.itemAction, id=btn.dataset.itemId;
    if(type==="note"){
      if(action==="edit")openNote(id);
      else if(action==="delete")deleteNoteById(id);
    }else if(type==="playlist"){
      if(action==="edit")editPlaylistById(id);
      else if(action==="delete")deletePlaylistById(id);
    }
    return;
  }
  closeAllItemMenus();
