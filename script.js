...
function savePlaylist() {
  ...
  if (editingPlaylistId) {
    ...
    // Favorites and Pinned depend on their exact names.
    const isSystem = playlist.name === "Favorites" || playlist.name === "Pinned";
    if (!isSystem) {
      if (!name) { alert("Enter a playlist name."); return; }
      if (playlists.some(p => p.id !== playlist.id && p.name.toLowerCase() === name.toLowerCase())) {
        alert("A playlist with this name already exists."); return;
      }
      playlist.name = name;
    }

    playlist.notes = [...$("playlistNoteSelector").querySelectorAll("input:checked")].map(x => x.value);
...
function openItemOptions(button) {
  ...
  // Favorites and Pinned are system playlists; editing their names would break their behavior.
  const playlist = type === "playlist" ? playlists.find(p => String(p.id) === String(itemId)) : null;
...
document.addEventListener("click", event => {
  const moreButton = event.target.closest("[data-more-type][data-more-id]");
  if (moreButton) {
    event.preventDefault();
    event.stopPropagation();
