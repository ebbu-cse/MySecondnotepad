const $ = id => document.getElementById(id);
const STORAGE_KEY = "MyNotesApp_Data_v3";
const LEGACY_KEY = "MyNotesApp_Offline_Data_v1";

function safeParse(value, fallback) {
  try { return value ? JSON.parse(value) : fallback; } catch { return fallback; }
}
function createDefaultData() {
  return { notes: [], playlists: [], lists: [], settings: { theme: "green", darkMode: false } };
}
function loadData() {
  const current = safeParse(localStorage.getItem(STORAGE_KEY), null);
  if (current && Array.isArray(current.notes) && Array.isArray(current.playlists)) {
    current.settings ||= { theme: "green", darkMode: false };
    current.lists ||= [];
    return current;
  }
  const legacy = safeParse(localStorage.getItem(LEGACY_KEY), null);
  if (legacy && Array.isArray(legacy.notes) && Array.isArray(legacy.playlists)) {
    return { ...legacy, lists: Array.isArray(legacy.lists) ? legacy.lists : [], settings: { theme: localStorage.getItem("theme") || "green", darkMode: localStorage.getItem("darkMode") === "true" } };
  }
  return createDefaultData();
}
let data = loadData();
data.lists = Array.isArray(data.lists) ? data.lists : [];
let notes = data.notes;
let playlists = data.playlists;
let lists = data.lists;
// Backfill creation timestamps for older notes so every note has a stable date.
notes.forEach(n => { if (!n.created) n.created = n.updated || Date.now(); });
let editingId = null;
let longPressNoteId = null;
let pressTimer = null;
let pressTriggered = false;
let activeItemMenu = null;
let pendingDelete = null;

let editingPlaylistId = null;

/* V15: Mobile-safe playlist/note options handler. */
function handleCardOptionsPress(event) {
  const button = event.target.closest(".card-more-btn[data-more-type][data-more-id]");
  if (!button) return;

  event.preventDefault();
  event.stopPropagation();
  openItemOptions(button);
}

document.addEventListener("pointerdown", handleCardOptionsPress, true);


// Dedicated options-button handler. This runs before the normal card click handlers.
document.addEventListener("click", event => {
  const moreButton = event.target.closest(".card-more-btn[data-more-type][data-more-id]");
  if (!moreButton) return;

  event.preventDefault();
  event.stopPropagation();
  openItemOptions(moreButton);
}, true);
notes.forEach(n => { if (typeof n.hidden !== "boolean") n.hidden = false; });

function saveData() {
  data.notes = notes;
  data.playlists = playlists;
  data.lists = lists;
  localStorage.setItem(STORAGE_KEY, JSON.stringify(data));
}
function id() { return `${Date.now()}-${Math.random().toString(16).slice(2)}`; }
function escapeHtml(s) { return String(s ?? "").replace(/[&<>"']/g, c => ({ "&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;" }[c])); }
function showModal(name) { $(name).classList.add("show"); }
function hideModal(name) {
  $(name).classList.remove("show");
  if (name === "noteModal") $("playlistDropdown").classList.remove("show");
  if (name === "playlistModal") {
    editingPlaylistId = null;
    $("playlistName").disabled = false;
    $("savePlaylistBtn").textContent = "Create Playlist";
  }
  if (name === "confirmDeleteModal") pendingDelete = null;
}

function dateKey(ts) {
  const d = new Date(ts || Date.now());
  return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
}
function dayLabel(key) {
  const now = new Date();
  const today = dateKey(now.getTime());
  const yesterday = new Date(now); yesterday.setDate(now.getDate()-1);
  const yKey = dateKey(yesterday.getTime());
  if (key === today) return "Today";
  if (key === yKey) return "Yesterday";
  const [y,m,d] = key.split("-").map(Number);
  return new Date(y,m-1,d).toLocaleDateString(undefined,{weekday:"long",day:"numeric",month:"long",year:"numeric"});
}
function formatCreated(ts) {
  return new Date(ts || Date.now()).toLocaleString(undefined,{day:"2-digit",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit"});
}
function groupedNotesHtml(items) {
  if (!items.length) return `<div class="empty">No notes found. Create your first note!</div>`;
  const groups = new Map();
  [...items].sort((a,b)=>(b.created||0)-(a.created||0)).forEach(n => {
    const key = dateKey(n.created);
    if (!groups.has(key)) groups.set(key, []);
    groups.get(key).push(n);
  });
  return [...groups.entries()].map(([key, dayNotes]) => `
    <div class="day-group">
      <div class="day-heading"><span>${dayLabel(key)}</span><small>${dayNotes.length} note${dayNotes.length===1?"":"s"}</small></div>
      <div class="notes-list">${dayNotes.map(n => `
        <article class="note-card ${n.pinned ? "pinned" : ""}" data-note="${n.id}">
          <button class="card-more-btn" data-more-type="note" data-more-id="${n.id}" aria-label="Note options">⋮</button>
          <span class="badge">${n.favorite ? "⭐ " : ""}${n.pinned ? "📌" : ""}</span>
          <h3>${escapeHtml(n.title || "Untitled")}</h3>
          <p>${escapeHtml(n.content || "Empty note").slice(0,130)}</p>
          <div class="note-meta">${escapeHtml(n.tags || "No tags")}</div>
          <div class="note-created">Created ${escapeHtml(formatCreated(n.created))}</div>
        </article>`).join("")}</div>
    </div>`).join("");
}
function render() {
  const searchBox = $("searchInput");
  const q = searchBox ? String(searchBox.value || "").trim().toLowerCase() : "";
  const filtered = notes.filter(n => {
    if (!n || n.hidden === true) return false;
    if (!q) return true;
    const searchable = `${n.title||""} ${n.tags||""} ${n.content||""}`.toLowerCase();
    return searchable.includes(q);
  });
  $("noteCount").textContent = `${filtered.length} note(s)`;
  $("notesList").innerHTML = groupedNotesHtml(filtered);

  $("playlists").innerHTML = playlists.length ? playlists.map(p => {
    const icon = playlistIcon(p.name), count = playlistNotes(p).length;
    return `<div class="playlist-card" data-playlist="${p.id}" role="button" tabindex="0">
      <button class="card-more-btn" data-more-type="playlist" data-more-id="${p.id}" aria-label="Playlist options">⋮</button>
      <div class="playlist-icon">${icon}</div><div class="playlist-info"><h3>${escapeHtml(p.name)}</h3><p>${count} note(s)</p></div>
    </div>`;
  }).join("") : `<div class="empty">No playlists yet.</div>`;
  $("sidebarPlaylists").innerHTML = playlists.map(p => `<button class="menu-item" data-playlist="${p.id}">${playlistIcon(p.name)} ${escapeHtml(p.name)}</button>`).join("");
  renderLists();
  saveData();
}
function playlistIcon(name) {
  if (name === "Favorites") return "⭐";
  if (name === "Pinned") return "📌";
  return "🎵";
}
function playlistNotes(p) {
  if (!p) return [];
  // Special playlists are calculated from the actual note status, so old saved IDs cannot break them.
  if (p.name === "Favorites") return notes.filter(n => n.hidden !== true && n.favorite === true);
  if (p.name === "Pinned") return notes.filter(n => n.hidden !== true && n.pinned === true);
  const ids = Array.isArray(p.notes) ? p.notes.map(String) : [];
  return notes.filter(n => n.hidden !== true && ids.includes(String(n.id)));
}


function renderHiddenNotes() {
  const container = $("hiddenNotesList");
  const hidden = notes.filter(n => n && n.hidden === true);
  if (!hidden.length) {
    container.innerHTML = `<div class="empty">No hidden notes.</div>`;
    return;
  }
  container.innerHTML = hidden.map(n => `
    <article class="note-card hidden-note-card" data-hidden-note="${n.id}">
      <h3>${escapeHtml(n.title || "Untitled")}</h3>
      <p>${escapeHtml(n.content || "Empty note").slice(0,130)}</p>
      <div class="note-meta">${escapeHtml(n.tags || "No tags")}</div>
    </article>`).join("");
}

function openHiddenNotes() {
  closeMenu();
  renderHiddenNotes();
  showModal("hiddenNotesModal");
}

function hideNote(noteId) {
  const targetId = String(noteId);
  const note = notes.find(n => n && String(n.id) === targetId);

  if (!note) {
    console.error("Hide failed: note not found", targetId);
    return;
  }

  // 1. Change the note data.
  note.hidden = true;
  note.updated = Date.now();

  // 2. Persist immediately.
  saveData();

  // 3. Remove the visible card immediately, even before a full re-render.
  document.querySelectorAll(`[data-note="${CSS.escape(targetId)}"]`).forEach(card => card.remove());

  // 4. Close menus and refresh all normal visible sections.
  longPressNoteId = null;
  hideModal("longPressMenu");
  hideModal("noteModal");
  render();
}
function unhideNote(noteId) {
  const n = notes.find(x => x && String(x.id) === String(noteId));
  if (!n) return;
  n.hidden = false;
  n.updated = Date.now();
  saveData();
  renderHiddenNotes();
  render();
}
function exportSingleNote(noteId) {
  const n = notes.find(x => x.id === noteId);
  if (!n) return;
  const text = `${n.title || "Untitled"}\n\nTags: ${n.tags || "None"}\n\n${n.content || ""}`;
  const blob = new Blob([text], {type:"text/plain"});
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = `${(n.title || "note").replace(/[\\/:*?"<>|]/g, "_")}.txt`;
  a.click();
  setTimeout(() => URL.revokeObjectURL(url), 500);
}
function openLongPressMenu(noteId) {
  longPressNoteId = noteId;
  showModal("longPressMenu");
}
function runLongPressAction(action) {
  const noteId = longPressNoteId;
  if (!noteId) return;
  const n = notes.find(x => x.id === noteId);
  if (!n) return;
  if (action === "edit") {
    hideModal("longPressMenu");
    openNote(noteId);
  } else if (action === "export") {
    exportSingleNote(noteId);
    hideModal("longPressMenu");
  } else if (action === "hide") {
    // Hide immediately and stop the menu click from reopening the note.
    hideNote(noteId);
    return;
  } else if (action === "pin") {
    n.pinned = !n.pinned;
    toggleSpecial(n.id, "Pinned", n.pinned);
    saveData();
    render();
    hideModal("longPressMenu");
  }
}

function getSpecial(name) {
  let p = playlists.find(x => x.name === name);
  if (!p) { p = { id: id(), name, notes: [] }; playlists.unshift(p); }
  return p;
}
function toggleSpecial(noteId, name, enabled) {
  const p = getSpecial(name);
  p.notes = enabled ? [...new Set([...p.notes, noteId])] : p.notes.filter(x => x !== noteId);
  if (!enabled && !p.notes.length) playlists = playlists.filter(x => x !== p);
}

function openNewNote() {
  editingId = null;
  $("editorHeading").textContent = "Create Note";
  $("noteTitle").value = "";
  $("noteTags").value = "";
  $("noteContent").value = "";
  $("noteCreatedInfo").textContent = "New note — creation time will be recorded when saved.";
  $("favoriteBtn").textContent = "☆ Favorite";
  $("pinBtn").textContent = "📌 Pin";
  showModal("noteModal");
}

function openNote(noteId) {
  const n = notes.find(x => String(x.id) === String(noteId));
  if (!n) return;

  editingId = n.id;
  $("editorHeading").textContent = "Edit Note";
  $("noteTitle").value = n.title || "";
  $("noteTags").value = n.tags || "";
  $("noteContent").value = n.content || "";
  $("noteCreatedInfo").textContent = `Created ${formatCreated(n.created)}`;
  $("favoriteBtn").textContent = n.favorite ? "⭐ Favorite" : "☆ Favorite";
  $("pinBtn").textContent = n.pinned ? "📌 Unpin" : "📌 Pin";
  showModal("noteModal");
}

function noteEditorHasContent() {
  return Boolean(
    $("noteTitle").value.trim() ||
    $("noteTags").value.trim() ||
    $("noteContent").value.trim()
  );
}

function saveCurrentNote(options = {}) {
  const { closeAfter = false, silent = false } = options;

  // A brand-new completely empty note is not created accidentally.
  if (!editingId && !noteEditorHasContent()) {
    if (closeAfter) hideModal("noteModal");
    return false;
  }

  let n = editingId ? notes.find(x => String(x.id) === String(editingId)) : null;

  if (!n) {
    n = {
      id: id(),
      favorite: false,
      pinned: false,
      hidden: false,
      created: Date.now()
    };
    notes.unshift(n);
    editingId = n.id;
  }

  n.title = $("noteTitle").value.trim() || "Untitled";
  n.tags = $("noteTags").value.trim();
  n.content = $("noteContent").value;
  n.updated = Date.now();

  saveData();
  render();

  if (!silent) {
    $("saveStatus").textContent = "Saved ✓";
    setTimeout(() => {
      if ($("saveStatus")) $("saveStatus").textContent = "";
    }, 1000);
  }

  if (closeAfter) {
    hideModal("noteModal");
  }

  return true;
}

function closeNoteAndSave() {
  // Back / X / close always preserves what the user has typed.
  saveCurrentNote({ closeAfter: true, silent: true });
}
function toggleFavorite() {
  if (!editingId) { alert("Save the note first."); return; }
  const n = notes.find(x => x.id === editingId); n.favorite = !n.favorite;
  toggleSpecial(n.id, "Favorites", n.favorite);
  $("favoriteBtn").textContent = n.favorite ? "⭐ Favorite" : "☆ Favorite"; saveData(); render();
}
function togglePin() {
  if (!editingId) { alert("Save the note first."); return; }
  const n = notes.find(x => x.id === editingId); n.pinned = !n.pinned;
  toggleSpecial(n.id, "Pinned", n.pinned);
  $("pinBtn").textContent = n.pinned ? "📌 Unpin" : "📌 Pin"; saveData(); render();
}
function openPlaylistModal(pid = null) {
  const existing = pid ? playlists.find(p => String(p.id) === String(pid)) : null;
  editingPlaylistId = existing ? existing.id : null;

  const heading = document.querySelector("#playlistModal .modal-top h2");
  if (heading) heading.textContent = existing ? "Edit Playlist" : "Create Playlist";

  $("playlistName").disabled = false;
  $("playlistName").value = existing ? (existing.name || "") : "";
  $("savePlaylistBtn").textContent = existing ? "Save Changes" : "Create Playlist";

  const selectedIds = new Set(existing && Array.isArray(existing.notes) ? existing.notes.map(String) : []);
  const visibleNotes = notes.filter(n => n && n.hidden !== true);
  $("playlistNoteSelector").innerHTML = visibleNotes.length
    ? visibleNotes.map(n => `<label class="selector-item"><input type="checkbox" value="${n.id}" ${selectedIds.has(String(n.id)) ? "checked" : ""}> ${escapeHtml(n.title || "Untitled")}</label>`).join("")
    : `<div class="empty">Create notes first.</div>`;

  showModal("playlistModal");
}

function savePlaylist() {
  const name = $("playlistName").value.trim();
  const selected = [...$("playlistNoteSelector").querySelectorAll('input[type="checkbox"]:checked')].map(x => String(x.value));

  if (editingPlaylistId) {
    const playlist = playlists.find(p => String(p.id) === String(editingPlaylistId));
    if (playlist) {
      if (!name) { alert("Please enter a playlist name."); return; }
      if (playlists.some(p => String(p.id) !== String(playlist.id) && String(p.name || "").toLowerCase() === name.toLowerCase())) {
        alert("A playlist with this name already exists."); return;
      }
      playlist.name = name;
      playlist.notes = selected;
      saveData();
      editingPlaylistId = null;
      hideModal("playlistModal");
      render();
      return;
    }
    editingPlaylistId = null;
  }

  if (!name) { alert("Please enter a playlist name."); return; }
  if (playlists.some(p => String(p.name || "").toLowerCase() === name.toLowerCase())) {
    alert("A playlist with this name already exists."); return;
  }

  playlists.push({ id:id(), name:name, notes:selected, created:Date.now() });
  saveData();
  editingPlaylistId = null;
  hideModal("playlistModal");
  render();
}
function viewPlaylist(pid) {
  const p = playlists.find(x => String(x.id) === String(pid)); if (!p) return;
  $("playlistViewTitle").textContent = playlistIcon(p.name) + " " + p.name;
  const ns = playlistNotes(p);
  $("playlistViewNotes").innerHTML = ns.length ? ns.map(n => `<article class="note-card" data-note="${n.id}"><h3>${escapeHtml(n.title)}</h3><p>${escapeHtml(n.content).slice(0,120)}</p></article>`).join("") : `<div class="empty">No notes in this playlist.</div>`;
  showModal("playlistViewModal");
}
function openPlaylistDropdown() {
  if (!editingId) { alert("Save the note first."); return; }
  $("playlistDropdown").innerHTML = playlists.length ? playlists.map(p => {
    const checked = p.notes.includes(editingId) ? "checked" : "";
    return `<label><input type="checkbox" data-add-playlist="${p.id}" ${checked}> ${escapeHtml(p.name)}</label>`;
  }).join("") : `<p style="padding:8px;margin:0">Create a playlist first.</p>`;
  $("playlistDropdown").classList.toggle("show");
}
function togglePlaylist(pid, checked) {
  const p = playlists.find(x => x.id === pid); if (!p || !editingId) return;
  if (checked) p.notes = [...new Set([...p.notes, editingId])];
  else p.notes = p.notes.filter(x => x !== editingId);
  saveData(); render();
}
function closeItemOptions() {
  const menu = $("itemOptionsMenu");
  menu.classList.remove("show");
  menu.style.display = "none";
  menu.setAttribute("aria-hidden", "true");
  activeItemMenu = null;
}
function openItemOptions(button) {
  const type = button.dataset.moreType;
  const itemId = button.dataset.moreId;
  if (!type || !itemId) return;

  activeItemMenu = { type, id: itemId };

  const menu = $("itemOptionsMenu");
  const editButton = menu.querySelector('[data-item-action="edit"]');
  const deleteButton = menu.querySelector('[data-item-action="delete"]');

  // Reset all menu buttons every time.
  editButton.style.display = "";
  deleteButton.style.display = "";

  const playlist = type === "playlist"
    ? playlists.find(p => String(p.id) === String(itemId))
    : null;

  if (playlist && (playlist.name === "Favorites" || playlist.name === "Pinned")) {
    // Keep special playlists safe from accidental renaming.
    editButton.style.display = "none";
  }

  const rect = button.getBoundingClientRect();
  const menuWidth = 170;
  const left = Math.max(8, Math.min(rect.right - menuWidth, window.innerWidth - menuWidth - 8));
  const top = Math.max(8, Math.min(rect.bottom + 7, window.innerHeight - 125));

  menu.style.left = `${left}px`;
  menu.style.top = `${top}px`;
  menu.style.display = "block";

  // Force the transition to start on the next frame.
  requestAnimationFrame(() => menu.classList.add("show"));
  menu.setAttribute("aria-hidden", "false");
}
function editActiveItem() {
  if (!activeItemMenu) return;
  const { type, id: itemId } = activeItemMenu;
  closeItemOptions();

  if (type === "note") {
    openNote(itemId);
  } else if (type === "playlist") {
    openPlaylistModal(itemId);
  }
}

function askDeleteActiveItem() {
  if (!activeItemMenu) return;
  pendingDelete = { ...activeItemMenu };
  closeItemOptions();

  const isNote = pendingDelete.type === "note";
  $("confirmDeleteTitle").textContent = isNote ? "Delete this note?" : "Delete this playlist?";
  $("confirmDeleteText").textContent = isNote
    ? "Are you sure you want to permanently delete this note? This action cannot be undone."
    : "Are you sure you want to permanently delete this playlist? Your notes themselves will not be deleted.";
  showModal("confirmDeleteModal");
}

function confirmDeleteItem() {
  if (!pendingDelete) { hideModal("confirmDeleteModal"); return; }

  const { type, id: itemId } = pendingDelete;

  if (type === "note") {
    const targetId = String(itemId);
    notes = notes.filter(n => String(n.id) !== targetId);
    playlists.forEach(p => {
      if (Array.isArray(p.notes)) p.notes = p.notes.filter(noteId => String(noteId) !== targetId);
    });
    if (String(editingId) === targetId) editingId = null;
  } else if (type === "playlist") {
    playlists = playlists.filter(p => String(p.id) !== String(itemId));
  }

  pendingDelete = null;
  saveData();
  render();
  hideModal("confirmDeleteModal");
}
function closeMenu() {
  $("sidebar").classList.remove("open");
  $("overlay").classList.remove("show");
  document.body.classList.remove("menu-open");
  closeItemOptions();
}
function openMenu() {
  $("sidebar").classList.add("open");
  $("overlay").classList.add("show");
  document.body.classList.add("menu-open");
  closeItemOptions();
}
function renderLists() {
  const c = $("listsContainer");
  if (!c) return;
  c.innerHTML = lists.length ? lists.map(list => `
    <div class="quick-list glass-panel" data-list-id="${list.id}">
      <div class="quick-list-head"><h3>☑️ ${escapeHtml(list.name)}</h3><button class="icon-btn list-delete" data-delete-list="${list.id}" aria-label="Delete list">🗑️</button></div>
      <div class="list-add-row"><input data-list-input="${list.id}" placeholder="Add an item..."><button class="small-btn" data-add-list-item="${list.id}">＋</button></div>
      <div class="list-items">${(list.items||[]).map((item,i)=>`<label class="list-item ${item.done?"done":""}"><input type="checkbox" data-toggle-list="${list.id}" data-item-index="${i}" ${item.done?"checked":""}><span>${escapeHtml(item.text)}</span><button data-delete-list-item="${list.id}" data-item-index="${i}" aria-label="Delete item">×</button></label>`).join("") || `<div class="list-empty">No items yet.</div>`}</div>
    </div>`).join("") : `<div class="empty">No lists yet. Create your first checklist.</div>`;
}
function createList() {
  const name = prompt("Enter list name:");
  if (!name || !name.trim()) return;
  lists.unshift({id:id(), name:name.trim(), items:[], created:Date.now()});
  saveData(); renderLists();
}
function addListItem(listId) {
  const input = document.querySelector(`[data-list-input="${CSS.escape(listId)}"]`);
  const text = input ? input.value.trim() : "";
  if (!text) return;
  const list = lists.find(x=>String(x.id)===String(listId));
  if (!list) return;
  list.items ||= []; list.items.push({text,done:false,created:Date.now()});
  input.value=""; saveData(); renderLists();
}
function toggleListItem(listId,index,checked) {
  const list=lists.find(x=>String(x.id)===String(listId)); if(!list) return;
  if(list.items?.[index]) list.items[index].done=checked;
  saveData(); renderLists();
}
function deleteListItem(listId,index) {
  const list=lists.find(x=>String(x.id)===String(listId)); if(!list) return;
  list.items.splice(Number(index),1); saveData(); renderLists();
}
function deleteList(listId) {
  if (!confirm("Delete this list?")) return;
  lists=lists.filter(x=>String(x.id)!==String(listId)); saveData(); renderLists();
}

let stopwatchRunning=false, stopwatchStartedAt=0, stopwatchElapsed=0, stopwatchRAF=null;
function stopwatchTick(){
  if(!stopwatchRunning) return;
  const ms=stopwatchElapsed+(performance.now()-stopwatchStartedAt);
  const total=Math.floor(ms/10), cs=total%100, sec=Math.floor(total/100)%60, min=Math.floor(total/6000)%60, hr=Math.floor(total/360000);
  $("stopwatchDisplay").textContent=`${String(hr).padStart(2,"0")}:${String(min).padStart(2,"0")}:${String(sec).padStart(2,"0")}.${String(cs).padStart(2,"0")}`;
  stopwatchRAF=requestAnimationFrame(stopwatchTick);
}
function toggleStopwatch(){
  if(stopwatchRunning){ stopwatchElapsed+=performance.now()-stopwatchStartedAt; stopwatchRunning=false; cancelAnimationFrame(stopwatchRAF); $("stopwatchStart").textContent="Resume"; }
  else { stopwatchStartedAt=performance.now(); stopwatchRunning=true; $("stopwatchStart").textContent="Pause"; stopwatchTick(); }
}
function resetStopwatch(){ stopwatchRunning=false; cancelAnimationFrame(stopwatchRAF); stopwatchElapsed=0; $("stopwatchDisplay").textContent="00:00:00.00"; $("stopwatchStart").textContent="Start"; }
let timerRemaining=60, timerRunning=false, timerLast=0, timerRAF=null;
function timerInputSeconds(){return Math.max(0,(Number($("timerMinutes").value)||0)*60+(Number($("timerSeconds").value)||0));}
function formatTimer(sec){return `${String(Math.floor(sec/60)).padStart(2,"0")}:${String(sec%60).padStart(2,"0")}`;}
function timerTick(now){ if(!timerRunning)return; if(!timerLast)timerLast=now; const elapsed=(now-timerLast)/1000; timerLast=now; timerRemaining=Math.max(0,timerRemaining-elapsed); $("timerDisplay").textContent=formatTimer(Math.ceil(timerRemaining)); if(timerRemaining<=0){timerRunning=false;timerLast=0;$("timerStart").textContent="Start";if(navigator.vibrate)navigator.vibrate([200,100,200]);alert("Timer finished!");return;} timerRAF=requestAnimationFrame(timerTick); }
function toggleTimer(){ if(timerRunning){timerRunning=false;timerLast=0;cancelAnimationFrame(timerRAF);$("timerStart").textContent="Resume";}else{if(timerRemaining<=0)timerRemaining=timerInputSeconds();if(timerRemaining<=0)return;timerRunning=true;timerLast=0;$("timerStart").textContent="Pause";timerRAF=requestAnimationFrame(timerTick);} }
function resetTimer(){timerRunning=false;timerLast=0;cancelAnimationFrame(timerRAF);timerRemaining=timerInputSeconds();$("timerDisplay").textContent=formatTimer(Math.ceil(timerRemaining));$("timerStart").textContent="Start";}
function switchTimeTab(tab){document.querySelectorAll(".time-tab").forEach(b=>b.classList.toggle("active",b.dataset.timeTab===tab));["clock","stopwatch","timer"].forEach(x=>$(x+"Panel").hidden=x!==tab);}
function updateClock(){const d=new Date();$("liveClock").textContent=d.toLocaleTimeString(undefined,{hour12:false});$("liveDate").textContent=d.toLocaleDateString(undefined,{weekday:"long",day:"numeric",month:"long",year:"numeric"});}
setInterval(updateClock,1000); updateClock();

function showView(view){
  const isNotes=view==="notes";
  ["calculatorView","listsView","timeView"].forEach(id=>$(id).hidden=true);
  document.querySelectorAll("#bottomNav .bottom-nav-item").forEach(b=>b.classList.toggle("active",b.dataset.view===view));
  if(view==="calculator")$("calculatorView").hidden=false;
  if(view==="lists"){$("listsView").hidden=false;renderLists();}
  if(view==="time")$("timeView").hidden=false;
  document.querySelectorAll("main > header, main > .search-section, main > section:not(.utility-view)").forEach(el=>el.hidden=!isNotes);
}

function applySettings() {
  document.body.dataset.theme = data.settings.theme || "green";
  document.body.classList.toggle("dark", !!data.settings.darkMode);
}
function exportNotes() {
  saveData();
  const blob = new Blob([JSON.stringify(data, null, 2)], {type:"application/json"});
  const url = URL.createObjectURL(blob); const a = document.createElement("a");
  a.href=url; a.download="MyNotesBackup.json"; a.click(); setTimeout(()=>URL.revokeObjectURL(url),500);
}

// Direct listeners for main controls
$("menuBtn").addEventListener("click", openMenu);
$("closeMenu").addEventListener("click", closeMenu);
$("overlay").addEventListener("click", closeMenu);
$("newNoteBtn").addEventListener("click", () => { closeMenu(); openNewNote(); });
$("headerNewNote").addEventListener("click", openNewNote);
$("createPlaylistBtn").addEventListener("click", () => { closeMenu(); openPlaylistModal(); });
$("createPlaylistBtn2").addEventListener("click", openPlaylistModal);
$("saveNoteBtn").addEventListener("click", () => saveCurrentNote({ closeAfter: true }));
$("favoriteBtn").addEventListener("click", toggleFavorite);
$("pinBtn").addEventListener("click", togglePin);
$("playlistMenuBtn").addEventListener("click", openPlaylistDropdown);
$("savePlaylistBtn").addEventListener("click", savePlaylist);
$("exportBtn").addEventListener("click", exportNotes);
$("itemOptionsMenu").addEventListener("click", event => {
  const actionButton = event.target.closest("[data-item-action]");
  if (!actionButton) return;
  event.preventDefault();
  event.stopPropagation();
  if (actionButton.dataset.itemAction === "edit") editActiveItem();
  if (actionButton.dataset.itemAction === "delete") askDeleteActiveItem();
});

$("confirmDeleteBtn").addEventListener("click", confirmDeleteItem);
$("cancelDeleteBtn").addEventListener("click", () => {
  pendingDelete = null;
  hideModal("confirmDeleteModal");
});
$("hiddenNotesBtn").addEventListener("click", (event) => {
  event.preventDefault();
  event.stopPropagation();
  closeMenu();

  const modal = $("hiddenNotesModal");
  const container = $("hiddenNotesList");
  const hidden = notes.filter(n => n && n.hidden === true);

  container.innerHTML = hidden.length
    ? hidden.map(n => `
      <article class="note-card hidden-note-card" data-hidden-note="${n.id}">
        <h3>${escapeHtml(n.title || "Untitled")}</h3>
        <p>${escapeHtml(n.content || "Empty note").slice(0,130)}</p>
        <div class="note-meta">${escapeHtml(n.tags || "No tags")}</div>
      </article>`).join("")
    : `<div class="empty">No hidden notes.</div>`;

  modal.classList.add("show");
});
$("searchInput").addEventListener("input", render);
$("themeToggle").addEventListener("click", () => {
  data.settings.darkMode = !data.settings.darkMode;
  applySettings(); saveData();
});

document.querySelectorAll("[data-theme-choice]").forEach(button => {
  button.addEventListener("click", event => {
    event.preventDefault();
    event.stopPropagation();
    data.settings.theme = button.dataset.themeChoice;
    applySettings(); saveData();
  });
});

// One reliable delegated click handler. This fixes dynamically rendered playlist cards too.
document.addEventListener("click", event => {
  if (!event.target.closest("#itemOptionsMenu")) closeItemOptions();

  const closeButton = event.target.closest("[data-close]");
  if (closeButton) {
    event.preventDefault();
    event.stopPropagation();

    if (closeButton.dataset.close === "noteModal") {
      closeNoteAndSave();
    } else {
      hideModal(closeButton.dataset.close);
    }
    return;
  }

  const playlistCheckbox = event.target.closest("[data-add-playlist]");
  if (playlistCheckbox) {
    togglePlaylist(playlistCheckbox.dataset.addPlaylist, playlistCheckbox.checked);
    return;
  }

  const noteCard = event.target.closest(".note-card[data-note]");
  if (noteCard) {
    if (pressTriggered || $("longPressMenu").classList.contains("show")) {
      event.preventDefault();
      return;
    }
    if (noteCard.closest("#playlistViewNotes")) hideModal("playlistViewModal");
    openNote(noteCard.dataset.note);
    return;
  }

  const playlistCard = event.target.closest("[data-playlist]");
  if (playlistCard) {
    if (event.target.closest(".card-more-btn")) return;
    closeMenu();
    viewPlaylist(playlistCard.dataset.playlist);
    return;
  }

  if (!event.target.closest(".dropdown")) $("playlistDropdown").classList.remove("show");
});

document.querySelectorAll(".modal").forEach(modal => {
  modal.addEventListener("click", event => {
    if (event.target !== modal) return;

    if (modal.id === "noteModal") {
      closeNoteAndSave();
    } else {
      hideModal(modal.id);
    }
  });
});

window.addEventListener("resize", closeItemOptions);
window.addEventListener("scroll", closeItemOptions, true);

document.addEventListener("keydown", event => {
  if (event.key === "Escape") {
    document.querySelectorAll(".modal.show").forEach(modal => {
      if (modal.id === "noteModal") closeNoteAndSave();
      else hideModal(modal.id);
    });
    closeMenu();
  }
});

function saveOpenNoteDraftSilently() {
  if ($("noteModal").classList.contains("show")) {
    saveCurrentNote({ closeAfter: false, silent: true });
  } else {
    saveData();
  }
}

window.addEventListener("pagehide", saveOpenNoteDraftSilently);
document.addEventListener("visibilitychange", () => {
  if (document.visibilityState === "hidden") saveOpenNoteDraftSilently();
});
window.addEventListener("storage", event => {
  if (event.key === STORAGE_KEY) {
    data = loadData(); notes = data.notes; playlists = data.playlists;
    applySettings(); render();
  }
});


$("longPressMenu").addEventListener("click", e => {
  const btn = e.target.closest("[data-note-action]");
  if (!btn) return;

  e.preventDefault();
  e.stopPropagation();

  const action = btn.dataset.noteAction;
  const selectedId = longPressNoteId;

  if (!selectedId) return;

  if (action === "hide") {
    hideNote(selectedId);
  } else {
    runLongPressAction(action);
  }
}, true);
$("hiddenNotesList").addEventListener("click", e => {
  const card = e.target.closest("[data-hidden-note]");
  if (!card) return;
  const noteId = card.dataset.hiddenNote;
  if (confirm("Unhide this note and show it again?")) unhideNote(noteId);
});

function startNotePress(noteId) {
  pressTriggered = false;
  clearTimeout(pressTimer);
  pressTimer = setTimeout(() => {
    pressTriggered = true;
    if (navigator.vibrate) navigator.vibrate(35);
    openLongPressMenu(noteId);
  }, 1000);
}
function cancelNotePress() {
  clearTimeout(pressTimer);
  pressTimer = null;
}
$("notesList").addEventListener("pointerdown", e => {
  const card = e.target.closest("[data-note]");
  if (!card) return;
  startNotePress(card.dataset.note);
});
$("notesList").addEventListener("pointerup", cancelNotePress);
$("notesList").addEventListener("pointerleave", cancelNotePress);
$("notesList").addEventListener("pointercancel", cancelNotePress);
$("notesList").addEventListener("click", e => {
  if (pressTriggered) {
    e.preventDefault();
    e.stopPropagation();
    pressTriggered = false;
  }
}, true);

document.querySelectorAll("#bottomNav .bottom-nav-item").forEach(btn=>btn.addEventListener("click",()=>showView(btn.dataset.view)));
$("newListBtn").addEventListener("click",createList);
$("listsContainer").addEventListener("click",e=>{
  const add=e.target.closest("[data-add-list-item]"); if(add){addListItem(add.dataset.addListItem);return;}
  const del=e.target.closest("[data-delete-list-item]"); if(del){deleteListItem(del.dataset.deleteListItem,del.dataset.itemIndex);return;}
  const dl=e.target.closest("[data-delete-list]"); if(dl){deleteList(dl.dataset.deleteList);return;}
});
$("listsContainer").addEventListener("change",e=>{const t=e.target.closest("[data-toggle-list]");if(t)toggleListItem(t.dataset.toggleList,t.dataset.itemIndex,t.checked);});
$("listsContainer").addEventListener("keydown",e=>{if(e.key==="Enter"&&e.target.matches("[data-list-input]"))addListItem(e.target.dataset.listInput);});
document.querySelectorAll("[data-calc]").forEach(btn=>btn.addEventListener("click",()=>{
  const d=$("calcDisplay"), v=btn.dataset.calc;
  if(v==="clear")d.value=""; else if(v==="back")d.value=d.value.slice(0,-1); else if(v==="="){try{if(!/^[0-9+\-*/%.() ]+$/.test(d.value))throw 0; d.value=String(Function("return ("+d.value+")")());}catch{d.value="Error";}} else {if(d.value==="Error")d.value="";d.value+=v;}
}));
$("calcDisplay").addEventListener("keydown",e=>{if(e.key==="Enter")document.querySelector('[data-calc="="]').click();});
$("stopwatchStart").addEventListener("click",toggleStopwatch); $("stopwatchReset").addEventListener("click",resetStopwatch);
$("timerStart").addEventListener("click",toggleTimer); $("timerReset").addEventListener("click",resetTimer); $("timerMinutes").addEventListener("input",()=>{if(!timerRunning)resetTimer();}); $("timerSeconds").addEventListener("input",()=>{if(!timerRunning)resetTimer();});
document.querySelectorAll("[data-time-tab]").forEach(b=>b.addEventListener("click",()=>switchTimeTab(b.dataset.timeTab)));

applySettings(); saveData(); render();

if ("serviceWorker" in navigator) {
  window.addEventListener("load", () => navigator.serviceWorker.register("./service-worker.js").catch(()=>{}));
}

/* ===== V13 reliable playlist creation handlers ===== */
document.addEventListener("click", function(event) {
  const createButton = event.target.closest("#createPlaylistBtn");
  if (createButton) {
    event.preventDefault();
    event.stopPropagation();
    event.stopImmediatePropagation();
    editingPlaylistId = null;
    openPlaylistModal();
    return;
  }

  const saveButton = event.target.closest("#savePlaylistBtn");
  if (saveButton) {
    event.preventDefault();
    event.stopPropagation();
    event.stopImmediatePropagation();
    savePlaylist();
  }
}, true);
