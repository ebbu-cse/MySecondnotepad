...
function playlistNotes(p) {
  ...
  if (p.name === "Favorites") return notes.filter(n => n.favorite === true);
  if (p.name === "Pinned") return notes.filter(n => n.pinned === true);
  const ids = Array.isArray(p.notes) ? p.notes.map(String) : [];
  return notes.filter(n => ids.includes(String(n.id)));
}


function renderHiddenNotes() {
  const hidden = notes.filter(n => n.hidden === true);
  $("hiddenNotesList").innerHTML = hidden.length ? hidden.map(n => `
    <article class="note-card hidden-note-card" data-hidden-note="${n.id}">
      <h3>${escapeHtml(n.title || "Untitled")}</h3>
      <p>${escapeHtml(n.content || "Empty note").slice(0,130)}</p>
      <div class="note-meta">${escapeHtml(n.tags || "No tags")}</div>
    </article>`).join("") : `<div class="empty">No hidden notes.</div>`;
}
...
function hideNote(noteId) {
  ...
  // Save first, then immediately remove the note from the visible UI.
  saveData();
  hideModal("longPressMenu");
  hideModal("noteModal");
  render();

  // Clear the long-press state so no later click can reopen the note.
